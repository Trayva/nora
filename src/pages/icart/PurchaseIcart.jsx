...
function PurchaseIcart() {
  ...
  const handlePurchase = async (settingsId) => {
    ...
    try {
      await api.post("/contract/application/purchase-icart", { settingsId, numberOfCarts });
  ...
  return (
    <div className="page_wrapper">
      ...
      {loading ? (
        ...
        <div className="icart_empty"><p>No contract settings available at the moment.</p></div>
      ) : (
        <div className="icart_settings_grid">
          {settings.map((setting) => {
            const isLoading = purchasing === setting.id;
            const totalAmount = setting.payments.reduce((sum, p) => sum + p.amount * numberOfCarts, 0);
              ...
              <div key={setting.id} className="icart_settings_card">
                ...
                <div className="icart_card_header">
                  <div>
                    ...
                    <span className="icart_card_location">{setting.country}</span>
                  </div>
                  <span className="icart_card_currency">{setting.currency}</span>
                </div>
                ...
                {setting.type !== "PURCHASE" && (
                  <div className="icart_card_meta">
                    ...
                    <span className="icart_meta_value">{setting.durationDays} days</span>
                  </div>
                )}
                  ...
                  {setting.payments.map((payment, i) => (
                    <div key={i} className="icart_payment_row">
                      <div className="icart_payment_info">
                        <span className="icart_payment_title">{payment.title}</span>
                        <span className="icart_payment_desc">{payment.description}</span>
                      </div>
                      <div className="icart_payment_right">
                        <span className="icart_payment_amount">
                          {setting.currency} {(payment.amount * numberOfCarts).toLocaleString()}
                        </span>
                        <div className="icart_payment_badges">
                          {payment.recurring && <span className="icart_badge icart_badge_recurring">Recurring</span>}
                          {payment.refundable && <span className="icart_badge icart_badge_refundable">Refundable</span>}
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="icart_qty_row">
                  ...
                  <div className="icart_qty_control">
                    <button className="icart_qty_btn" onClick={() => setNumberOfCarts((n) => Math.max(1, n - 1))} disabled={numberOfCarts === 1}>-</button>
                    <span className="icart_qty_value">{numberOfCarts}</span>
                    <button className="icart_qty_btn" onClick={() => setNumberOfCarts((n) => n + 1)}>+</button>
                  </div>
                </div>

                {/* Total */}
                <div className="icart_card_total">
                  <span className="icart_total_label">Total for {numberOfCarts} iCart{numberOfCarts > 1 ? "s" : ""}</span>
                  <span className="icart_total_amount">{setting.currency} {totalAmount.toLocaleString()}</span>
                </div>
