...
export default function CreateExtraModal({ isOpen, onClose, onSuccess }) {
  const [form, setForm] = useState({ name: "", unit: "", recipeOutput: "", description: "" });
...
const handleSubmit = async (e) => {
  ...
  if (!form.recipeOutput || isNaN(Number(form.recipeOutput)) || Number(form.recipeOutput) <= 0)
      ...
      <form onSubmit={handleSubmit}>
        <div className="modal-body">
          <div className="form-field">
            ...
            <input className="modal-input" placeholder="e.g. Pepper Mix"
              value={form.name} onChange={f("name")} required />
          </div>
          <div className="form-field">
            <label className="modal-label">Description</label>
            <input className="modal-input" placeholder="e.g. My signature pepper mix"
              value={form.description} onChange={f("description")} />
          </div>
          <div className="register_row">
            <div className="form-field">
              <label className="modal-label">Unit of Measure *</label>
              <input className="modal-input" placeholder="e.g. g, ml, kg"
                value={form.unit} onChange={f("unit")} required />
            </div>
            <div className="form-field">
              <label className="modal-label">Recipe Output *</label>
              <input className="modal-input" type="number" placeholder="e.g. 300"
                value={form.recipeOutput} onChange={f("recipeOutput")} required />
            </div>
          </div>
          <div className="modal-footer">
            <button className="app_btn app_btn_cancel" type="button" onClick={onClose}>Cancel</button>
            <button className={`app_btn app_btn_confirm ${loading ? "btn_loading" : ""}`}
              type="submit" disabled={loading} style={{ position: "relative", minWidth: 120 }}>
