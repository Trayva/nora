...
import {
  ...
  MdOutlineLocalShipping, MdOutlineFactCheck, MdOutlineLocationOn,
  MdClose, MdCheck, MdCircle, MdArrowForward, MdOutlineTrendingUp, MdOutlineHub,
} from "react-icons/md";
...
export default function AdminDashboard() {
  ...
  // Stats
  const [stats, setStats] = useState({ users: 0, vendors: 0, operators: 0, icarts: 0, suppliers: 0, aggregators: 0 });
  ...
  const fetchStats = async () => {
    ...
    try {
      const [usersR, vendorsR, operatorsR, icartsR, suppliersR, aggregatorsR] = await Promise.allSettled([
        api.get("/account"),
      ...
      const count = (r) => {
        ...
        // named array key
        const k = ["vendors","operators","suppliers","users","icarts","states","aggregators"].find((k) => Array.isArray(d[k]));
        if (k) return d[k].length;
        if (Array.isArray(d)) return d.length;
        return d.items?.length || 0;
      };
      setStats({ users: count(usersR), vendors: count(vendorsR), operators: count(operatorsR), icarts: count(icartsR), suppliers: count(suppliersR), aggregators: count(aggregatorsR) });
    } catch { /* silent */ }
  ...
  const DRAWER_CONFIG = {
    users:     { url: "/account",             title: "All Users",      description: "Every registered user across all roles" },
    ...
    aggregators: { url: "/icart/aggregator",     title: "Aggregators",    description: "All iCart aggregators" },
  };

  const openDrawer = async (key) => {
    // Users and icarts have dedicated full drawers
    if (key === "users")  { setUsersOpen(true);  return; }
    if (key === "icarts")     { setIcartsOpen(true);     return; }
    if (key === "operators") { setOperatorsOpen(true); return; }
    ...
    try {
      ...
      const extract = (d) => {
        ...
        const namedKey = ["vendors","operators","suppliers","users","icarts","states","aggregators","data"].find((k) => Array.isArray(d?.[k]));
  ...
  const handleCreateAggregator = async () => {
    ...
    finally { setSavingAgg(false); }
  };

  const handleAssignCart = async (aggregatorId) => {
    if (!assignCartId.trim()) return toast.error("Enter an iCart serial number");
    setAssigningCart(true);
    try {
      // Resolve serial number to cart ID
      const searchRes = await api.get("/icart", { params: { search: assignCartId.trim() } });
      const d = searchRes.data.data;
      const list = Array.isArray(d) ? d : d?.items || d?.icarts || [];
      const cart = list.find((c) => c.serialNumber?.toLowerCase() === assignCartId.trim().toLowerCase());
      if (!cart) { toast.error("iCart not found — check the serial number"); setAssigningCart(false); return; }
      await api.post("/icart/aggregator/assign-cart", { aggregatorId, cartId: cart.id });
      toast.success("Cart assigned");
      setShowAssignForm(null);
      setAssignCartId("");
      openDrawer("aggregators");
    } catch (err) { toast.error(err.response?.data?.message || "Failed"); }
    finally { setAssigningCart(false); }
  };
  ...
  const STAT_CARDS = [
    ...
    { key: "aggregators", label: "Aggregators", icon: MdOutlineHub,            color: "#06b6d4" },
  ];
  ...
  const renderEntityRow = (item) => {
    ...
    return (
      <div key={item.id} className="admin_drawer_row" style={{ cursor: drawer === "vendors" ? "pointer" : "default" }}
        onClick={() => { if (drawer === "vendors") { setDrawer(null); setSelectedVendor(item); } }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
          ...
          {isUsersDrawer && item.passwordLockedUntil && (
            <button
              ...
              style={{ height: 28, padding: "0 10px", fontSize: "0.72rem", position: "relative", whiteSpace: "nowrap" }}
    ...
    <div className="admin_dashboard">
      ...
      <div className="admin_stats_grid">
        {STAT_CARDS.map(({ key, label, icon: Icon, color }) => (
          <div key={key} className="admin_stat_card" onClick={() => {
      ...
      <div className="admin_section">
        ...
        <div className="admin_approval_row">
          {[
            ...
            <button key={key} className="admin_approval_chip" onClick={() => key === "applications" ? setAppsDrawerOpen(true) : openDrawer(key)}>
      ...
      <div className="admin_section" id="admin_apps_section">
        <div className="admin_section_header">
          ...
          {appsPending > 0 && (
            <span className="admin_section_count" style={{ background: "rgba(234,179,8,0.1)", color: "#ca8a04", border: "1px solid rgba(234,179,8,0.25)" }}>
              {appsPending} pending
            </span>
          )}
          <button className="app_btn app_btn_cancel" style={{ height: 30, padding: "0 12px", fontSize: "0.75rem", marginLeft: "auto" }}
        ...
        {appsLoading ? (
          ...
          <>
                  ...
                  <div key={app.id} className="admin_card">
                    ...
                    <div className="admin_card_body">
                      ...
                      <div className="admin_card_meta">
                        {(user?.fullName || user?.name) && <span className="admin_meta_chip">{user.fullName || user.name}</span>}
            ...
            {applications.length > 4 && (
              <button className="app_btn app_btn_cancel" style={{ width: "100%", height: 38, marginTop: 10, fontSize: "0.8rem", fontWeight: 700 }}
                onClick={() => setAppsDrawerOpen(true)}>
      ...
      <div className="admin_section">
        ...
        <div
          ...
          style={{ flexDirection: "row", alignItems: "center", gap: 16, cursor: "pointer", padding: "18px 20px" }}
          onClick={() => setAnalyticsOpen(true)}
        >
          <div className="admin_stat_icon" style={{ color: "var(--accent)", width: 44, height: 44, borderRadius: 12, flexShrink: 0 }}>
            <MdOutlineTrendingUp size={22} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "0.9rem", fontWeight: 800, color: "var(--text-heading)", marginBottom: 3 }}>
              Platform Sales
            </div>
            <div style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>
              Revenue trends, payment breakdown, top iCarts and full transaction history
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
            <span style={{ fontSize: "0.72rem", fontWeight: 700, padding: "3px 10px", borderRadius: 999, background: "var(--bg-active)", color: "var(--accent)", border: "1px solid rgba(203,108,220,0.2)" }}>
              View Analytics
            </span>
      ...
      {/* Users drawer */}
      <Drawer isOpen={usersOpen} onClose={() => setUsersOpen(false)} title="All Users" description="Every registered user on the platform" width={560}>
        <AdminUsers />
      </Drawer>

      {/* Operators drawer */}
      <AdminOperators open={operatorsOpen} onClose={() => setOperatorsOpen(false)} />

      {/* iCarts drawer */}
      <Drawer isOpen={icartsOpen} onClose={() => setIcartsOpen(false)} title="iCart Fleet" description="All iCarts — filter by state or serial number" width={600}>
        <AdminIcarts />
      </Drawer>

      {/* Vendor detail drawer */}
      {selectedVendor && (
        <AdminVendorDetail vendor={selectedVendor} onClose={() => { setSelectedVendor(null); openDrawer("vendors"); }} />
      )}
      ...
      <AdminApplications
        ...
        onApproved={() => { fetchApplications(); fetchStats(); }}
      />
            ...
            {showAggForm ? (
              <div className="admin_form_card">
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                  <span style={{ fontSize: "0.82rem", fontWeight: 700, color: "var(--text-heading)" }}>Create Aggregator</span>
                  <button className="biz_icon_btn" onClick={() => setShowAggForm(false)}><MdClose size={13} /></button>
                </div>
                <div className="form-field" style={{ marginBottom: 10 }}>
                  <label className="modal-label">Search User *</label>
                  {selectedUser ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.25)", borderRadius: 9 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(6,182,212,0.15)", color: "#06b6d4", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "0.82rem", flexShrink: 0 }}>
                        {(selectedUser.fullName || selectedUser.email || "U").charAt(0).toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: "0.8rem", fontWeight: 700, color: "var(--text-heading)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{selectedUser.fullName || "User"}</div>
                        <div style={{ fontSize: "0.68rem", color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{selectedUser.email}</div>
                      </div>
                      <button onClick={() => { setSelectedUser(null); setUserSearch(""); setUserResults([]); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", padding: 0, display: "flex" }}>
                        <MdClose size={14} />
                      </button>
                    </div>
                  ) : (
                    <div style={{ position: "relative" }}>
                      ...
                      {userSearching && (
                        <div style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)" }}>
                          <div className="page_loader_spinner" style={{ width: 14, height: 14 }} />
                        </div>
                      )}
                      {userResults.length > 0 && (
                        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 50, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", boxShadow: "0 6px 24px rgba(0,0,0,0.12)" }}>
                          {userResults.map((u) => (
                            <div
                              key={u.id}
                              onClick={() => { setSelectedUser(u); setUserSearch(""); setUserResults([]); }}
                              style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", cursor: "pointer", borderBottom: "1px solid var(--border)", transition: "background 0.1s" }}
                              onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"}
                              onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}
                            >
                              <div style={{ width: 28, height: 28, borderRadius: 7, background: "var(--bg-active)", border: "1px solid rgba(203,108,220,0.2)", color: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "0.75rem", flexShrink: 0 }}>
                                {(u.fullName || u.email || "U").charAt(0).toUpperCase()}
                              </div>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: "0.78rem", fontWeight: 700, color: "var(--text-heading)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.fullName || "—"}</div>
                                <div style={{ fontSize: "0.68rem", color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.email}</div>
                              </div>
                              {u.roles?.length > 0 && (
                                <span style={{ fontSize: "0.6rem", fontWeight: 700, padding: "1px 6px", borderRadius: 999, background: "var(--bg-hover)", border: "1px solid var(--border)", color: "var(--text-muted)", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                  <button className="app_btn app_btn_cancel" style={{ height: 34 }} onClick={() => setShowAggForm(false)}>Cancel</button>
                  <button className={`app_btn app_btn_confirm${savingAgg ? " btn_loading" : ""}`} style={{ height: 34, minWidth: 80, position: "relative" }} onClick={handleCreateAggregator} disabled={savingAgg}>
                    <span className="btn_text">Create</span>
                    {savingAgg && <span className="btn_loader" style={{ width: 12, height: 12 }} />}
                  </button>
                </div>
              </div>
            ) : (
              <button className="app_btn app_btn_confirm biz_add_btn" style={{ marginBottom: 12 }} onClick={() => setShowAggForm(true)}>
            ...
            {showLocationForm ? (
              <div className="admin_form_card">
                ...
                <div className="admin_form_grid" style={{ marginBottom: 10 }}>
                  ...
                  <div className="form-field" style={{ marginBottom: 0 }}><label className="modal-label">Country</label><CountrySelect value={locationForm.country} onChange={(e) => setLocationForm((p) => ({ ...p, country: e.target.value }))} /></div>
          ...
          <div className="admin_drawer_list">
            {drawerItems.map((item) => {
              if (drawer === "aggregators") {
                const name = item.user?.fullName || item.user?.name || item.businessName || item.userId?.slice(0, 8);
                const email = item.user?.email || "";
                // API doesn't return carts on aggregator — cross-reference from icartList
                const carts = icartList.filter((c) => c.aggregatorId === item.id);
                const initials = (name || "A").charAt(0).toUpperCase();
                return (
                  <div key={item.id}>
                    <div className="admin_drawer_row" style={{ flexDirection: "column", alignItems: "stretch", gap: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div className="admin_drawer_avatar" style={{ background: "rgba(6,182,212,0.12)", color: "#06b6d4", border: "1px solid rgba(6,182,212,0.2)" }}>{initials}</div>
                        <div className="admin_drawer_info">
                          <div className="admin_drawer_name">{name || "Aggregator"}</div>
                          {email && <div className="admin_drawer_sub">{email}</div>}
                        </div>
                        <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                          <span className="admin_meta_chip">{carts.length} cart{carts.length !== 1 ? "s" : ""}</span>
                          <button className="app_btn app_btn_confirm biz_add_btn" style={{ height: 28 }}
                            onClick={() => { setShowAssignForm(showAssignForm === item.id ? null : item.id); setAssignCartId(""); }}>
                      ...
                      {carts.length > 0 && (
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, paddingLeft: 46 }}>
                          {carts.map((c) => (
                            <div key={c.id || c} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 9px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 999, fontSize: "0.7rem", fontWeight: 700, color: "var(--text-body)", fontFamily: "monospace" }}>
                              {c.serialNumber || c.id?.slice(0, 8).toUpperCase() || c}
                              <button onClick={() => handleUnassignCart(c.id || c)} style={{ background: "none", border: "none", cursor: "pointer", color: "#ef4444", padding: 0, display: "flex", alignItems: "center", lineHeight: 1 }}>
                                <MdClose size={11} />
                              </button>
                            </div>
                          ))}
                      ...
                      {showAssignForm === item.id && (
                        <div style={{ paddingLeft: 46, display: "flex", gap: 8, alignItems: "center" }}>
                          <input
                            className="modal-input"
                            style={{ flex: 1, height: 34, marginBottom: 0, fontFamily: "monospace" }}
                            placeholder="Enter iCart serial number…"
                            value={assignCartId}
                            onChange={(e) => setAssignCartId(e.target.value)}
                          />
                          <button className={`app_btn app_btn_confirm${assigningCart ? " btn_loading" : ""}`} style={{ height: 34, padding: "0 14px", position: "relative", flexShrink: 0 }} onClick={() => handleAssignCart(item.id)} disabled={assigningCart || !assignCartId}>
                            <span className="btn_text">Assign</span>
                            {assigningCart && <span className="btn_loader" style={{ width: 12, height: 12 }} />}
                          </button>
                          <button className="app_btn app_btn_cancel" style={{ height: 34, padding: "0 10px", flexShrink: 0 }} onClick={() => setShowAssignForm(null)}>
                            <MdClose size={14} />
                          </button>
                        </div>
                      )}
